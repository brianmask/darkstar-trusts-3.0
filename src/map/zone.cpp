...
// TODO:
// ????? ????????? ????? czone ?? ??????? ? ???????????. ??? ????????????: Standard, Rezident, Instance ? Dinamis
// ? ?????? ?? ????????? ??? ?????? ?????????
...
void CZone::LoadZoneSettings()
{
        ...
        Sql_NextRow(SqlHandle) == SQL_SUCCESS)
    {
        ...
        if (Sql_GetData(SqlHandle,10) != nullptr) // ?????? ?????? ???????????? bcnmid, ?.?. ??? ?????????? ? ????
        {
...
void CZone::DeletePET(CBaseEntity* PPet)
{
    ...
    ShowWarning(CL_RED"REGULAR Delete PET\n" CL_RESET);
}

/************************************************************************
*                                                                       *
*  ????????? ? ???? PET (????????? targid 0x700-0x7FF)                  *
*                                                                       *
************************************************************************/
...
void CZone::InsertPET(CBaseEntity* PPet)
{
    ...
    ShowWarning(CL_RED"REGULAR INSERT PET\n" CL_RESET);
}
...
void CZone::CharZoneOut(CCharEntity* PChar)
{
    ...
    if (PChar->PTreasurePool != nullptr) // TODO: ??????? ??? ?????????? ??????? ? MobHouse, ???? ???? ?????? ?? ??? ? ????????
    {
